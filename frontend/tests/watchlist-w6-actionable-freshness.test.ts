...
test("W6: helper de frescura cubre caso sin timestamp ni snapshot", () => {
  ...
  assert.match(source, /Sin datos todavía|watchlist\.freshness\.noDataLabel/);
});
...
test("W6: i18n agrega copy de frescura accionable en ES", () => {
  ...
  assert.match(source, /noDataLabel:\s*"Sin datos todavía"/);
  assert.match(source, /noDataDetail:\s*"actualiza para crear el primer snapshot"/);
  assert.match(source, /needsReviewLabel:\s*"Necesita revisión"/);
  assert.match(source, /lastUpdatedAgo:\s*"última actualización \{time\}"/);
  assert.match(source, /updatedAgo:\s*"actualizado \{time\}"/);
  assert.match(source, /observingLabel:\s*"En observación"/);
});
